...
```{r, fig.width = 20*0.393701, fig.height = 26*0.393701, fig.dpi = 300}
p1 <- bd |> 
...
(eligible response variable distribution)
```{r}
...
```{r, fig.width = 7, fig.height = 3, fig.cap = "Distribution of the countries from which the data of the 55 selected articles come from."}
bd |> 
  ggplot(aes(x = fct_rev(fct_infreq(Country)))) +
  geom_bar(fill = "skyblue") +
  geom_text(aes(label = paste0(stat(count), 
                               " (", round(prop.table(count)*100, 1), "%)")), stat = "count", 
            hjust = -0.2) +
  coord_flip() +
...
```{r, fig.width=23*0.393701, fig.height = 28*0.393701, fig.cap = "Distribution of the sports studied (panel (A)), the gender of the participants (panel (B)) and the professional category of the participants (panel (C)) in the 55 selected articles.", fig.pos="H", out.extra=""}
...
```{r, fig.width = 23*0.393701, fig.height = 27*0.393701, fig.cap="Distribution of the type of study design (panel (A)), whether multilevel models are used (panel (B)) and of the statistical software used (panel (C)) in the 55 selected articles.", fig.pos = "H", out.extra=""}
